...
export function ModelSelector({
  ...
  const cerebrasModels = AVAILABLE_MODELS.filter(
  ...
  const getThinkingBadge = (model: ModelConfig) => {
    ...
    const badgeStyles: Record<string, string> = {
      ...
      medium: "bg-indigo-500/20 text-indigo-600 dark:text-indigo-400",
  ...
  return (
    <Select value={value} onValueChange={onValueChange}>
      <SelectTrigger
        className={cn(
          "h-9 w-[240px] border border-border bg-card shadow-sm text-xs font-medium hover:bg-accent transition-all duration-300 rounded-lg text-muted-foreground hover:text-foreground focus:ring-0 focus:ring-offset-0",
          className,
        )}
      >
        <SelectValue placeholder="Select Model" />
      </SelectTrigger>
      <SelectContent className="max-h-[400px] bg-popover border border-border rounded-xl shadow-md p-1">
