...
import { Brush, Laptop2, MoonStar, SunMedium, Settings, FileText, Type, Code2, Server, RefreshCw } from "lucide-react";
...
import { getLSPBinaryStatuses, installLSPBinary, type LSPBinaryStatus } from "@/services/lsp/lspBinaryService";
...
const SettingsPage = () => {
  ...
  const [currentView, setCurrentView] = useState<"quick" | "appearance" | "editor" | "explorer" | "lsp" | "fonts" | "advanced">("quick");
  ...
  const [lspInstallHistory, setLspInstallHistory] = useState<LSPInstallHistoryEntry[]>([]);

  const pushLspHistory = useCallback((entry: Omit<LSPInstallHistoryEntry, "id" | "timestamp">) => {
  ...
  const refreshLspBinaries = useCallback(async () => {
    ...
    try {
      ...
      notificationActions.error(`Failed to load LSP binaries: ${message}`, { source: "lsp" });
    } finally {
      setLspLoading(false);
    }
  }, []);
  ...
  }, [currentView, refreshLspBinaries]);

  const handleInstallLspBinary = useCallback(async (server: LSPBinaryStatus) => {
    ...
    try {
      ...
      if (result.installed) {
        notificationActions.success(result.message, { source: "lsp", autoHide: true, autoHideDelay: 5000 });
        pushLspHistory({
      ...
      const message = error instanceof Error ? error.message : String(error);
      notificationActions.error(`Failed to install ${server.name}: ${message}`, { source: "lsp" });
      pushLspHistory({
        serverId: server.server_id,
        serverName: server.name,
        status: "failed",
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Manage native language server binaries used for external LSP support.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  <p className="text-sm text-muted-foreground">
                    Monaco built-in handles TS/JS. External binaries are used for Rust, Python, Go and others.
                  </p>
                  <Button
                    variant="outline"
                    size="sm"
                    ...
                    disabled={lspLoading}
                  >
                    <RefreshCw className={`mr-2 h-4 w-4 ${lspLoading ? "animate-spin" : ""}`} />
                    Re-check
                  </Button>
                </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {lspBinaries.map((server) => (
                        <tr key={server.server_id} className="border-t border-border/70">
                          <td className="px-3 py-2">
                            <div className="font-medium">{server.name}</div>
                            <div className="text-xs text-muted-foreground">{server.instructions}</div>
                          </td>
                          <td className="px-3 py-2 font-mono text-xs">{server.command}</td>
                          <td className="px-3 py-2">
                            {server.installed ? (
                              <span className="text-emerald-500 font-medium">Installed</span>
                            ) : (
                          ...
                          <td className="px-3 py-2 text-right">
                            <Button
                              ...
                              size="sm"
                              disabled={server.installed || installingServerId === server.server_id}
                              onClick={() => void handleInstallLspBinary(server)}
                            >
                              {installingServerId === server.server_id ? "Installing..." : server.installed ? "Installed" : "Install"}
                            </Button>
                          </td>
                        </tr>
                      ))}
            ...
            <Card>
              ...
              <CardContent>
                {lspInstallHistory.length === 0 ? (
                  <p className="text-sm text-muted-foreground">No installation attempts yet.</p>
                ) : (
                      ...
                      {lspInstallHistory.map((entry) => (
                        <div key={entry.id} className="p-3 space-y-2">
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-2">
                              <span className="font-medium text-sm">{entry.serverName}</span>
                              <Badge
                                variant={
                          ...
                          {entry.output && (
                            <pre className="text-xs p-2 rounded-md bg-muted/40 overflow-x-auto whitespace-pre-wrap break-words">
                              {entry.output.length > 1000 ? `${entry.output.slice(0, 1000)}...` : entry.output}
                            </pre>
                          )}
                        </div>
                      ))}
