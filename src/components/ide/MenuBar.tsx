...
import {
  ...
  useNativeMenu,
  MenuEventHandlers,
} from "../../hooks/useNativeMenuEvents";
  ...
  const nativeMenuHandlers: MenuEventHandlers = useMemo(
    () => ({
      ...
      "selection:add-next-occurrence": () =>
        editorActions.addSelectionToNextFindMatch(),
      "selection:select-all-occurrences": () =>
        editorActions.selectAllMatches(),
      ...
      // Terminal menu
      "terminal:new": () =>
        terminalActions.createSession({ cwd: state().workspace?.path }),
      "terminal:kill": () => {
      ...
      "help:getting-started": async () => {
        ...
        await invoke("open_external_url", {
          url: "https://github.com/ferxalbs/rainy-aether#readme",
        });
      },
      "help:documentation": async () => {
        const { invoke } = await import("@tauri-apps/api/core");
        await invoke("open_external_url", {
          url: "https://github.com/ferxalbs/rainy-aether/wiki",
        });
      },
      "help:release-notes": async () => {
        const { invoke } = await import("@tauri-apps/api/core");
        await invoke("open_external_url", {
          url: "https://github.com/ferxalbs/rainy-aether/releases",
        });
      },
      ...
      "help:report-issue": async () => {
        const { invoke } = await import("@tauri-apps/api/core");
        await invoke("open_external_url", {
          url: "https://github.com/ferxalbs/rainy-aether/issues/new",
        });
      },
      "help:github": async () => {
        const { invoke } = await import("@tauri-apps/api/core");
      ...
      "help:check-updates": () => openUpdateModal(),
    }),
    [
      actions,
      state,
    ...
    <div className="h-10 border-b border-border flex items-center w-full bg-background select-none">
      <div className="flex items-center h-full">
        ...
        <Menubar className="h-full border-none rounded-none px-1 py-0 gap-1 shadow-none bg-transparent">
          <MenubarMenu>
            <MenubarTrigger className="h-7 px-3 text-sm font-normal">
              File
            </MenubarTrigger>
            <MenubarContent align="start" className="min-w-56 py-1 text-sm">
                  ...
                  <MenubarSubContent className="min-w-52 py-1 text-sm">
                    {recentWorkspaces.map((workspace) => (
                      <MenubarItem
                        ...
                        onSelect={() => {
                          const current = state();
                          if (current.workspace?.path === workspace.path)
                            return;
                          actions.openRecentWorkspace(workspace);
                        }}
                      >
                        {workspace.name}
                        {snapshot.workspace?.path === workspace.path
                          ? " (current)"
                          : ""}
                      </MenubarItem>
                    ))}
              ...
              <MenubarItem
                disabled={!hasActiveFile}
                onSelect={() => actions.revealActiveFile()}
              >
                Reveal Active File in Explorer
              </MenubarItem>
              <MenubarItem
                disabled={!hasWorkspace}
                onSelect={() => actions.revealWorkspace()}
              >
              ...
              <MenubarItem
                onSelect={() => actions.setAutoSave(!state().autoSave)}
              >
          ...
          <MenubarMenu>
            <MenubarTrigger className="h-7 px-3 text-xs font-normal">
              Edit
            </MenubarTrigger>
            ...
            <MenubarContent align="start" className="min-w-56 py-1 text-sm">
              ...
              <MenubarSub>
                ...
                <MenubarSubContent className="min-w-52 py-1 text-sm">
                  ...
                  <MenubarItem
                    onSelect={() => editorActions.toggleBreadcrumbs()}
                  >
                    Toggle Breadcrumbs
                  </MenubarItem>
                </MenubarSubContent>
              ...
              <MenubarSeparator />
              <MenubarItem
                onSelect={() => actions.setSidebarActive("explorer")}
              >
                Explorer
              ...
              <MenubarSeparator />
              <MenubarItem
                onSelect={() => panelActions.togglePanel("terminal")}
              >
                Terminal
                <MenubarShortcut>{cmdKey}+`</MenubarShortcut>
              </MenubarItem>
              <MenubarItem
                onSelect={() => panelActions.togglePanel("problems")}
              >
                Problems
          ...
          <MenubarMenu>
            ...
            <MenubarContent align="start" className="min-w-56 py-1 text-sm">
              ...
              <MenubarItem
                onSelect={() => editorActions.addSelectionToNextFindMatch()}
              >
                Add Next Occurrence
          ...
          <MenubarMenu>
            ...
            <MenubarContent align="start" className="min-w-48 py-1 text-sm">
              ...
              <MenubarItem
                disabled={!hasWorkspace}
                onSelect={() => actions.setSidebarActive("git")}
              >
                Open Source Control
              </MenubarItem>
            </MenubarContent>
          ...
          <MenubarMenu>
            ...
            <MenubarContent align="start" className="min-w-52 py-1 text-sm">
              ...
              <MenubarItem
                ...
                onSelect={() => {
                  if (activeTerminalId)
                    terminalActions.removeSession(activeTerminalId);
                }}
              >
                Kill Terminal
            ...
            <MenubarContent align="start" className="min-w-56 py-1 text-sm">
              ...
              <MenubarItem
                onSelect={async () => {
                  const { invoke } = await import("@tauri-apps/api/core");
                  await invoke("open_external_url", {
                    url: "https://www.enosislabs.com/rainy-aether",
                  });
                }}
              >
                Visit Our Website
              ...
              <MenubarSeparator />
              <MenubarItem onSelect={() => openUpdateModal()}>
                Check for Updates…
              </MenubarItem>
            </MenubarContent>
          </MenubarMenu>
