 ...
 * Manages external Language Server Protocol (LSP) servers for non-Monaco languages
 *
 * This module provides the Rust backend for spawning and communicating with external
 * language servers. While Monaco Editor handles TypeScript/JavaScript natively, this
 * infrastructure enables support for additional languages:
...
struct LanguageServerProcess {
    /// Process handle
    child: Child,
    /// Server ID (stored for debugging/logging purposes)
    #[allow(dead_code)]
    server_id: String,
...
/// On Windows, npm-installed binaries need .cmd extension
fn resolve_command_path(command: &str) -> String {
    #[cfg(target_os = "windows")]
    {
        // On Windows, try .cmd extension for npm binaries
        if !command.ends_with(".cmd") && !command.ends_with(".exe") {
            // Check if this is a common npm binary that needs .cmd
            let npm_bins = [
            ...
            if npm_bins.iter().any(|b| command.contains(b)) {
...
impl LanguageServerManager {
    ...
    pub fn start_server(
        ...
        // Resolve command for cross-platform compatibility (Windows .cmd extension)
        ...
        // Spawn the process with improved error handling
        let mut child = match cmd.spawn() {
            ...
            Err(e) => {
                ...
                if e.kind() == std::io::ErrorKind::NotFound {
                    eprintln!(
                        "[LSP] Command not found: {} (resolved: {})",
                        params.command, resolved_command
                    );
                    return Err(LSPError::CommandNotFound(format!(
        ...
        let stdout = child.stdout.take().ok_or(LSPError::StdioCaptureFailed)?;
        let stderr = child.stderr.take().ok_or(LSPError::StdioCaptureFailed)?;
    ...
    pub fn send_message(&self, server_id: &str, message: &str) -> Result<(), LSPError> {
        ...
        if let Some(server_process) = servers.get_mut(server_id) {
            if let Some(stdin) = &mut server_process.stdin {
                ...
                stdin.flush().map_err(LSPError::MessageSendFailed)?;
